...
export default function GpaView() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) toast.error(t('failedToLoadGpaData'));
      } finally {
  ...
  async function handleCreate() {
    if (!form.courseName) return toast.error(t('courseNameRequired'));
    setSaving(true);
    try {
    ...
    } catch { toast.error(t('failedToAddCourse')); }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{t('gpaPredictor')}</h1>
          <p className="text-sm mt-1" style={{ color: '#6B7280' }}>{t('gpaPredictorDescription')}</p>
        </div>
          ...
          [
            {
              ...
              color: currentGpa !== null ? gpaColor(currentGpa) : '#6B7280',
              sub: t('basedOnCurrentGrades'),
            },
            {
              label: t('predictedGpa'),
              value: predictedGpa !== null ? predictedGpa.toFixed(2) : 'N/A',
              color: predictedGpa !== null ? gpaColor(predictedGpa) : '#6B7280',
              sub: t('withStudyHoursFactoredIn'),
            },
            {
              label: t('targetGpa'),
              value: targetGpa !== null ? targetGpa.toFixed(2) : 'N/A',
              color: '#4F46E5',
              sub: t('yourGoalThisSemester'),
            },
            {
              label: t('totalCredits'),
              value: totalCredits.toString(),
              color: '#06B6D4',
              sub: t('coursesEnrolled', { count: entries.length }),
            },
          ].map(c => (
      ...
      {currentGpa !== null && (
        <div className="rounded-2xl p-6 mb-6" style={{ background: '#FFFFFF', border: '1px solid #E5E7EB' }}>
          <h2 className="font-semibold mb-4 flex items-center gap-2">
            ...
            {t('gpaForecastVisualization')}
          </h2>
          <div className="space-y-4">
            {
              [
                { label: t('currentGpa'), value: currentGpa, color: gpaColor(currentGpa) },
                { label: t('predictedGpa'), value: predictedGpa || currentGpa, color: '#4F46E5' },
                { label: t('targetGpa'), value: targetGpa || currentGpa, color: '#06B6D4' },
              ].map(item => (
                <div key={item.label}>
          ...
          <div className="mt-4 p-3 rounded-xl" style={{ background: '#FAFAFA', border: '1px solid #E5E7EB' }}>
            <p className="text-xs" style={{ color: '#6B7280' }}>
              <span style={{ color: '#4F46E5' }}>{t('aiInsight')}: </span>
              {predictedGpa !== null && currentGpa !== null && predictedGpa > currentGpa
                ? t('gpaBoostInsight', { points: (predictedGpa - currentGpa).toFixed(2) })
                : t('gpaImprovementInsight', { hours: Math.ceil(totalStudyHours / Math.max(entries.length, 1) + 2) })
              }
            </p>
          </div>
        </div>
      ...
      <div className="rounded-2xl" style={{ background: '#FFFFFF', border: '1px solid #E5E7EB' }}>
        <div className="px-5 py-4" style={{ borderBottom: '1px solid #E5E7EB' }}>
          <h2 className="font-semibold flex items-center gap-2">
            ...
            {t('enrolledCourses', { count: entries.length })}
          </h2>
        </div>
        {entries.length === 0 ? (
          <div className="text-center py-12">
            <GraduationCap size={40} color="#E5E7EB" className="mx-auto mb-3" />
            <p style={{ color: '#6B7280' }}>{t('noCoursesAdded')}</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: '1px solid #E5E7EB' }}>
                  {[t('course'), t('credits'), t('currentGrade'), t('studyHrsWk'), t('target'), t('gpaPoints'), ''].map(h => (
              ...
              <tbody>
                {entries.map(entry => {
                  ...
                  return (
                    <tr key={entry.id} className="hover:bg-gray-50 transition-colors" style={{ borderBottom: '1px solid #E5E7EB' }}>
                      <td className="px-4 py-3">
                        <p className="text-sm font-medium">{entry.courseName}</p>
                        <p className="text-xs" style={{ color: '#6B7280' }}>{entry.semester}</p>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {isEditing ? (
                          <input type="number" min="0" max="100" value={editGrade}
                            ...
                            style={{ background: '#FAFAFA', border: '1px solid #4F46E5', color: '#111827' }} />
                        ) : (
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold" style={{ color: gpaColor(points) }}>{entry.currentGrade}%</span>
                            <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: `${gpaColor(points)}15`, color: gpaColor(points) }}>{letterGrade(grade)}</span>
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => handleDelete(entry.id)} className="p-1.5 rounded-lg" style={{ color: '#6B7280' }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{ background: '#FFFFFF', border: '1px solid #E5E7EB' }}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-semibold text-lg">{t('addCourse')}</h3>
              <button onClick={() => setShowForm(false)}><X size={20} color="#6B7280" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>{t('courseName')} *</label>
                <input value={form.courseName} onChange={e => setForm(p => ({ ...p, courseName: e.target.value }))}
                  placeholder={t('e.g.CalculusIII')}
                  className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                  style={{ background: '#FAFAFA', border: '1px solid #E5E7EB', color: '#111827' }} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>{t('credits')}</label>
                ...
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>{t('semester')}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>{t('currentGrade')} (%)</label>
                ...
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>{t('targetGrade')} (%)</label>
              ...
              <div>
                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>{t('studyHoursPerWeek')}</label>
