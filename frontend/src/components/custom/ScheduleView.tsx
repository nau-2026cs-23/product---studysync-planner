...
export default function ScheduleView() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) toast.error(t('failedToLoadSchedule'));
      } finally {
  ...
  async function handleCreate() {
    if (!form.title || !form.date) return toast.error(t('titleAndDateRequired'));
    setSaving(true);
    try {
    ...
    } catch { toast.error(t('failedToCreateSession')); }
  ...
  async function toggleComplete(s: StudySession) {
    ...
    } catch { toast.error(t('failedToUpdateSession')); }
  }
  ...
  async function handleDelete(id: string) {
    ...
    } catch { toast.error(t('failedToDeleteSession')); }
  }
  ...
  function generateAISessions() {
    ...
    setShowForm(true);
    toast.success(t('aiRecommendationLoaded'), { description: t('reviewAndSaveSession') });
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: '#6B7280' }}>Plan and manage your study sessions</p>
        </div>
      ...
      <div className="flex items-center justify-between mb-4 rounded-xl px-4 py-3" style={{ background: '#FFFFFF', border: '1px solid #E5E7EB' }}>
        <button onClick={() => setWeekOffset(w => w - 1)} className="p-1.5 rounded-lg transition-colors hover:opacity-70" style={{ color: '#6B7280' }}>
          <ChevronLeft size={18} />
        </button>
        <div className="text-center">
          <p className="text-sm font-semibold">{weekLabel}</p>
          <p className="text-xs" style={{ color: '#6B7280' }}>Week {weekOffset === 0 ? '(Current)' : weekOffset > 0 ? `+${weekOffset}` : weekOffset}</p>
        </div>
        <button onClick={() => setWeekOffset(w => w + 1)} className="p-1.5 rounded-lg transition-colors hover:opacity-70" style={{ color: '#6B7280' }}>
        ...
        {weekDates.map((date, i) => {
          ...
          return (
            <div key={date} className="rounded-xl p-3 min-h-[120px]" style={{
              background: isToday ? 'rgba(79,70,229,0.08)' : '#FFFFFF',
              border: isToday ? '1px solid rgba(79,70,229,0.3)' : '1px solid #E5E7EB',
            }}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold" style={{ color: isToday ? '#4F46E5' : '#6B7280' }}>{DAY_LABELS[i]}</span>
                <span className="text-xs" style={{ color: '#6B7280' }}>{date.slice(5)}</span>
              </div>
              <div className="space-y-1">
                {daySessions.map(s => (
                  <div key={s.id} className="rounded-lg px-2 py-1 text-xs" style={{
                    background: `${SESSION_COLORS[s.type] || '#4F46E5'}15`,
                    border: `1px solid ${SESSION_COLORS[s.type] || '#4F46E5'}30`,
                    color: SESSION_COLORS[s.type] || '#4F46E5',
                    textDecoration: s.completed ? 'line-through' : 'none',
                    opacity: s.completed ? 0.6 : 1,
                  }}>
                    <div className="font-medium truncate">{s.title}</div>
                    <div style={{ color: '#6B7280' }}>{s.startTime} · {s.durationHours}h</div>
                  </div>
                ))}
                {dayDeadlines.map(d => (
                  <div key={d.id} className="rounded-lg px-2 py-1 text-xs" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#EF4444' }}>
      ...
      <div className="rounded-2xl" style={{ background: '#FFFFFF', border: '1px solid #E5E7EB' }}>
        ...
        {sessions.length === 0 ? (
          <div className="text-center py-12">
            <CalendarDays size={40} color="#E5E7EB" className="mx-auto mb-3" />
            <p style={{ color: '#6B7280' }}>No sessions yet. Add your first study session!</p>
          </div>
        ) : (
              ...
              return (
                <div key={s.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: `${color}15`, color }}>{s.type}</span>
                    <button onClick={() => handleDelete(s.id)} className="p-1 rounded-lg transition-colors" style={{ color: '#6B7280' }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{ background: '#FFFFFF', border: '1px solid #E5E7EB' }}>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>Title *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>Date *</label>
                  <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))}
                    className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                    style={{ background: '#FAFAFA', border: '1px solid #E5E7EB', color: '#111827' }} />
                </div>
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>Type</label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>Start</label>
                  <input type="time" value={form.startTime} onChange={e => setForm(p => ({ ...p, startTime: e.target.value }))}
                    className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                    style={{ background: '#FAFAFA', border: '1px solid #E5E7EB', color: '#111827' }} />
                </div>
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>End</label>
                  <input type="time" value={form.endTime} onChange={e => setForm(p => ({ ...p, endTime: e.target.value }))}
                    className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                    style={{ background: '#FAFAFA', border: '1px solid #E5E7EB', color: '#111827' }} />
                </div>
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>Hours</label>
              ...
              {courses.length > 0 && (
                <div>
                  <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>Course (optional)</label>
              ...
              <div>
                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#6B7280' }}>Notes</label>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.aiRecommended} onChange={e => setForm(p => ({ ...p, aiRecommended: e.target.checked }))}
                  className="rounded" />
                <span className="text-sm" style={{ color: '#6B7280' }}>Mark as AI Recommended</span>
              </label>
            </div>
